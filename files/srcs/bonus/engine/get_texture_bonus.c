...
/*   Updated: 2020/07/19 01:10:36 by mmateo-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				get_texture(t_engine *en)
{
	...
	en->tex.x = (int)(en->tex.wall_x *
	(double)(en->mlx.texture[en->tex.num].width));
	...
	en->tex.pos = (en->draw_start - g_config.r.y / 2 +
	en->line_height / 2) * en->tex.step;
	return (0);
}
...
static void		fc_casting_calculate(t_engine *en, int y)
{
	...
	en->cf.floor_step.x = en->cf.row_distance * (en->cf.ray_dir_1.x -
	en->cf.ray_dir_0.x) / g_config.r.x;
	en->cf.floor_step.y = en->cf.row_distance * (en->cf.ray_dir_1.y -
	en->cf.ray_dir_0.y) / g_config.r.x;
	en->cf.floor.x = en->pos.x + en->cf.row_distance * en->cf.ray_dir_0.x;
	en->cf.floor.y = en->pos.y + en->cf.row_distance * en->cf.ray_dir_0.y;
}

static void		calculation_to_draw(t_engine *en)
{
	en->cf.cell.x = (int)(en->cf.floor.x);
	en->cf.cell.y = (int)(en->cf.floor.y);
	en->cf.t.x = (int)(en->mlx.texture[4].width *
	(en->cf.floor.x - en->cf.cell.x)) & (en->mlx.texture[4].width - 1);
	en->cf.t.y = (int)(en->mlx.texture[4].height *
	(en->cf.floor.y - en->cf.cell.y)) & (en->mlx.texture[4].height - 1);
...
void			floor_casting(t_engine *en)
{
	...
	while (y < g_config.r.y)
	{
		...
		{
			calculation_to_draw(en);
			en->color = (int)en->mlx.texture[5].data
			[(int)(en->mlx.texture[5].width * en->cf.t.y + en->cf.t.x)];
			en->color = dark_color(en->cf.row_distance, en->color);
			*(en->mlx.img.data + (y * g_config.r.x) + x) = en->color;
			en->color = (int)en->mlx.texture[4].data
			[(int)(en->mlx.texture[4].width * en->cf.t.y + en->cf.t.x)];
			en->color = dark_color(en->cf.row_distance, en->color);
			*(en->mlx.img.data + ((g_config.r.y - y - 1) *
			g_config.r.x) + x) = en->color;
			x++;
		}
		y++;
	}
