...
/*   Updated: 2020/07/17 12:55:49 by mmateo-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			put_sprite_to_image(t_engine *en, t_sprite_cast s, int stripe)
{
	...
	tex.x = (int)(256 * (stripe - (-s.sprite_width / 2 + s.sprite_screen_x)) * en->mlx.texture[4].width / s.sprite_width) / 256;
	if (s.transform.y > 0 && stripe > 0 && stripe < g_config.R.x && s.transform.y < en->z_buff[stripe])
	{
		...
		while (y < s.draw_end.y)
		{
			...
			en->color = en->mlx.texture[4].data[en->mlx.texture[4].width * tex.y + tex.x];
